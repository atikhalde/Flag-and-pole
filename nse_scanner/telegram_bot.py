...
SENT: list = []
FAILED: list = []
BOT_USERNAME: str = ""   # filled in by the first getMe (whoami / a successful send)
...
>>>>>>> Stashed changes
    ...
    print(f"  [telegram] !!! NOT DELIVERED after {retries} attempt(s) to chat {_mask(chat)}: "
          f"{FAILED[-1]['reason']} - this is printed in the run summary too, and the next run retries.")
    return False

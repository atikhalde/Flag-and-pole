...
def _run_summary(results, buys, in_trade, waiting, gated, regime, mode, sent, digest_sent, mcap_map,
    ...
    if TG.SENT:
        ...
        L.append(f"- the bot is **@{TG.BOT_USERNAME or 'Flagandpole_bot'}** - if that is not the chat you "
                 f"read, message the bot from the app you actually use and re-run with "
                 f"`diag_telegram: true`; the log will show your real chat id")
    elif TG.FAILED:
        f0 = TG.FAILED[0]
        ...
        L.append("- check `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID` (Settings -> Secrets -> Actions); "
                 "the exact error is above and the next run retries automatically")
    else:
